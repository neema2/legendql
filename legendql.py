...
class LegendQL:
    ...
    def left_join(self, join: LegendQL, condition: Callable, select: Optional[Callable]) -> LegendQL:

        parsed = parser.Parser.parse_join(condition, self.query, join.query)
        clause = JoinClause(right=join.query, condition=parsed, type=JoinType())

        self.query.clauses.append(clause)

        clause = SelectClause(parser.Parser.parse(select, self.query))
        self.query.clauses.append(clause)
