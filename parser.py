...
class Parser:
    ...
    @staticmethod
    def parse_join(func: Callable, query: Query, join: Query) -> Expression:
        """
    ...
    def _parse_expression(node: ast.AST, alias: dict) -> Union[Expression, List[Expression]]:
        ...
        if isinstance(node, ast.Compare):
            ...
            if node.func.id == 'over':
                return OverFunction(args_list[0], args_list[1], kwargs.get("sort"), kwargs.get("frame"), kwargs.get("filter"))
            elif node.func.id == "aggregate":
                return AggregateFunction(args_list[0], args_list[1], kwargs.get("filter"))
